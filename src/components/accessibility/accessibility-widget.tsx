        ...
        <DialogContent className="max-w-[850px] w-[95vw] max-h-[85vh] h-[650px] p-0 border border-border shadow-2xl rounded-2xl gap-0 bg-background flex flex-col overflow-hidden">
          ...
          <div className="flex-1 overflow-hidden md:grid md:grid-cols-[1.6fr_1fr] bg-background">
            ...
            <div className="h-full overflow-y-auto custom-scrollbar p-6 space-y-8 bg-background">
                        ...
                        <div className="bg-background border border-border rounded-xl p-1 relative group hover:border-primary/50 transition-colors">
                            ...
                            >
                                ...
                                <optgroup label="Tricromacia (Parcial)">
                                    <option value="protanomaly">Vermelho Fraco (Protanomalia)</option>
                                    <option value="deuteranomaly">Verde Fraco (Deuteranomalia)</option>
                                    <option value="tritanomaly">Azul Fraco (Tritanomalia)</option>
                                </optgroup>
                                <optgroup label="Dicromacia (Total)">
                                    <option value="protanopia">Sem Vermelho (Protanopia)</option>
                                    <option value="deuteranopia">Sem Verde (Deuteranopia)</option>
                                    <option value="tritanopia">Sem Azul (Tritanopia)</option>
                                </optgroup>
                                <optgroup label="Monocromacia">
                                    <option value="achromatopsia">Acromatopsia (Escala de Cinza)</option>
                                </optgroup>
                            </select>
                ...
                <section className="space-y-4">
                    <SectionHeader icon={<Type className="h-4 w-4" />} title="Tipografia & Leitura" />
                    ...
                    <div className="bg-background border border-border rounded-xl p-3 space-y-3">
                        ...
                        <div className="space-y-2">
                            ...
                            <div className="flex gap-1 bg-muted/20 p-1 rounded-lg">
                                {['normal', 'wide', 'wider'].map((s) => (
                                    <Button key={s} variant="ghost" className={cn("flex-1 h-7 text-xs rounded-md transition-all hover:bg-background", state.letterSpacing === s && "bg-background shadow-sm text-primary font-bold ring-1 ring-border")} onClick={() => {
                                        updateState('letterSpacing', s);
                                        updateState('lineHeight', s === 'wider' ? 'loose' : s === 'wide' ? 'relaxed' : 'normal');
                                    }}>
                                        {s === 'normal' ? 'Normal' : s === 'wide' ? 'Largo' : 'Extra'}
                                    </Button>
                                ))}
                            </div>
                        </div>
                ...
                <section className="space-y-4">
                    <SectionHeader icon={<Brain className="h-4 w-4" />} title="Cognitivo & Motor" />
                    <div className="grid gap-2">
                        <ToggleRow 
                            icon={<MousePointer2 className="h-4 w-4" />} 
                            label="Cursor Ampliado" 
                            checked={state.cursorBig} 
                            onChange={(c) => updateState('cursorBig', c)} 
                        />
                        <ToggleRow 
                            icon={<Focus className="h-4 w-4" />} 
                            label="Foco de Teclado Forte" 
                            checked={state.focusStrong} 
                            onChange={(c) => updateState('focusStrong', c)} 
                        />
                        <ToggleRow 
                            icon={<Activity className="h-4 w-4" />} 
                            label="Reduzir Movimento" 
                            checked={state.reduceMotion} 
                            onChange={(c) => updateState('reduceMotion', c)} 
                        />
                        <ToggleRow 
                            icon={<ImageIcon className="h-4 w-4" />} 
                            label="Ocultar Imagens (Foco)" 
                            checked={state.hideImages} 
                            onChange={(c) => updateState('hideImages', c)} 
                        />
                    </div>
                </section>

                <section className="space-y-3 pt-2 pb-6">
                    <SectionHeader icon={<Check className="h-4 w-4" />} title="Em Breve (Roadmap)" />
                    
                    <div className="grid grid-cols-1 gap-2 opacity-50 pointer-events-none">
                        <ComingSoonRow icon={<Hand className="h-4 w-4" />} label="VLibras / Tradutor de Sinais" />
                        <ComingSoonRow icon={<Volume2 className="h-4 w-4" />} label="Leitor de Tela (TTS Web)" />
                        <ComingSoonRow icon={<MessageSquare className="h-4 w-4" />} label="Legendas Automáticas" />
                        <ComingSoonRow icon={<Globe className="h-4 w-4" />} label="Tradução de Idioma" />
                        <ComingSoonRow icon={<Fingerprint className="h-4 w-4" />} label="Login Biométrico" />
                        <ComingSoonRow icon={<Keyboard className="h-4 w-4" />} label="Atalhos de Teclado Custom" />
                    </div>
                </section>
...
function SectionHeader({ icon, title }: { icon: React.ReactNode, title: string }) {
    return (
        <h4 className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-widest text-muted-foreground pl-1">
            {icon} {title}
        </h4>
    )
}
...
function ThemeCard({ active, onClick, icon, label }: { active: boolean, onClick: () => void, icon: React.ReactNode, label: string }) {
    return (
        <button 
            onClick={onClick}
            className={cn(
                "flex flex-col items-center justify-center gap-2 p-3 rounded-xl border transition-all duration-200 group relative overflow-hidden h-20 bg-background hover:bg-muted/30",
                active 
                    ? "border-primary text-primary ring-1 ring-primary/20 bg-primary/5" 
...
function ToggleRow({ icon, label, checked, onChange }: { icon: React.ReactNode, label: string, checked: boolean, onChange: (c: boolean) => void }) {
    return (
        <div 
            className={cn(
                "flex items-center justify-between p-3 rounded-xl border transition-all duration-200 cursor-pointer group h-12 bg-background hover:bg-muted/30",
                checked ? "border-primary/50 bg-primary/5" : "border-border"
            )}
            onClick={() => onChange(!checked)}
        >
            <div className="flex items-center gap-3">
                <div className={cn("h-7 w-7 rounded-full flex items-center justify-center shrink-0 transition-colors", checked ? "bg-primary text-primary-foreground" : "bg-muted/20 text-muted-foreground group-hover:text-primary")}>
                    {icon}
                </div>
...
function ComingSoonRow({ icon, label }: { icon: React.ReactNode, label: string }) {
    return (
        <div className="flex items-center justify-between p-3 rounded-xl border border-dashed border-border bg-muted/5 h-10 select-none">
            <div className="flex items-center gap-3">
                <div className="h-5 w-5 rounded-full flex items-center justify-center bg-muted/20 text-muted-foreground">
                    {icon}
                </div>
